...
The full model follows the architecture from the [supplementary material](https://openreview.net/attachment?id=fyL9HD-kImm&name=supplementary_material):
...
#### ResNet structure (`GEResNetBlock`)
Each block applies two MHSA layers in pre-norm style (LayerNorm → attention → residual). GELayerNorm normalizes jointly over channels and the N group dimension, sharing affine parameters across N to preserve equivariance.
...
**Current best result: 82% test accuracy** (30 training epochs vs 70 reported by the authors — accuracy is likely improvable with longer training).
